...
class Environment:  # 定义环境类
    ...
    def step(self, actions):
        ...
        if completion_rate < self.completion_rate_threshold:
            for i in range(len(rewards)):
                rewards[i] += self.reward_params['late_task']
...
class MultiAgent:
    ...
    def __init__(self, state_size, action_size, n_agents, epsilon=1.0, min_epsilon=0.01, high_reward_threshold=100.0):
        ...
        self.actor = Actor(state_size, action_size)  # 创建Actor网络
        self.critic = Critic(state_size)  # 创建Critic网络
    ...
    # 定义如何根据状态选择动作的方法
    def act(self, state):
        state_tensor = torch.from_numpy(state[0].astype(np.float32))
